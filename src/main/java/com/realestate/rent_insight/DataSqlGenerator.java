...
/**
 * 국토교통부 법정동코드 원본 CSV 파일에서, 우리가 필요로 하는 '서울특별시'의 지역 데이터만 INSERT 하기
 * '법정동코드.csv' 파일을 읽기
...
public class DataSqlGenerator {
    ...
    public static void main(String[] args) {
        ...
        // .sql 파일에 집어넣으면 Spring Boot 에서 뚝딱 Insert 해줌
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            // 엑셀 첫번째 줄은 컬럼명이기에 한줄 넘기려고 사용해씀둥
            br.readLine();

            // 파일을 한 줄씩 읽기
            while ((line = br.readLine()) != null) {
                // 각 줄을 쉼표(,)로 나누기 "1111000000,서울특별시,종로구,"
                String[] columns = line.split(",");

                // 컬럼 순서에 맞게 데이터를 변수에 담음
                String code = columns[0];
                String sido = columns[1];
                String sigungu = (columns.length > 2) ? columns[2] : "";        // 1100000000,서울특별시,
                String eupmyeondong = (columns.length > 3) ? columns[3] : "";   // 1111000000,서울특별시,종로구,xxxx
