...
public class CallApiDBLogicService {
...
//    private final CallApiService callApiService;
    ...
    @Transactional
    public ApiCountDTO callDataProcess(String dealYmd) {
...
//        String dealYmd = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMM"));
        ...
        // 준비한 URL 2개라서  2번 굴려야함 ㅋ,ㅋ
        List<String> baseUrlList = new ArrayList<>();

        // 오피스텔 URL
        baseUrlList.add(apiProperties.getBaseUrl().getOfficetel());

        // 연립다세대 URL
        baseUrlList.add(apiProperties.getBaseUrl().getMultiHouse());

        for (String finalBaseUrl : baseUrlList) {
            log.info(">>>> {} API 데이터 수집을 시작합니다. ");

            for (Region sigungu : regionList) {
                try {
                    // LAWD_CD 잘라내기 앞 5자리
                    String lawdCd = sigungu.getCode().substring(0, 5);
                    log.info(">>>>> 데이터 수집중... {} 시군구명 : {} , 시군구 5자리 : {}",
                            finalBaseUrl.contains("Offi") ? "오피스텔" : "연립다세대" ,
                            sigungu.getName(),
                            lawdCd);

                    // 호출 _ UriComponentsBuilder 완전한 요청URL 만들기
                    URI uri = UriComponentsBuilder.fromUriString(finalBaseUrl)
                    ...
                    if (!dtoList.isEmpty()) {
                        ...
                        for (ApiRentDTO dto : dtoList) {
                            RentComplete entity = this.apiRentDtoConversionRentComplete(dto,sigungu);
                    ...
                    throw new RuntimeException("데이터 수집 실패 (지역: " + sigungu.getName() + ")", e);
                }
            }
        }
        return new ApiCountDTO(totalDeletedCount, totalInsertedCount);
    }

    public RentComplete apiRentDtoConversionRentComplete(ApiRentDTO apiRentDTO , Region sigungu) {
        ...
        String finalSggNm = apiRentDTO.getSggNm();
        if(finalSggNm == null || finalSggNm.isBlank()) {
            finalSggNm = sigungu.getName();
        }
